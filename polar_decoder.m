...
for i = 1:N
    ...
    for listIdx = 1:list_size
        ...
        for s = n:-1:1
            % perform f operation
            idx = trellis(s,:);
            idx = idx(~(isnan(LLR(idx,s+1)) | isnan(LLR(idx+2^(s-1),s+1))) & isnan(LLR(idx,s)));
            LLR(idx,s) = f(LLR(idx,s+1),LLR(idx+2^(s-1),s+1),1);
            % perform g operation
            idx = trellis(s,:);
            idx = idx(~isnan(par_sum(idx, s)) & isnan(LLR(idx+2^(s-1),s)));
            LLR(idx+2^(s-1),s) = g(par_sum(idx,s),LLR(idx,s+1),LLR(idx+2^(s-1),s+1));
        end
